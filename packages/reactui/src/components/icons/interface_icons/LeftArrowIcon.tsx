...
const OutlinedIcon = ({ fill, className, ...props }: IconProps) => {
  const classNames = ['fscLeftArrow_outlinedIcon', className && `${className}`].filter(Boolean).join(' ')
  return (
    <svg
      {...props}
...
const LeftArrowOutlinedIcon = withLayout(OutlinedIcon)

export { LeftArrowOutlinedIcon }
