...
const OutlinedIcon = ({ fill, className, ...props }: IconProps) => {
  const classNames = ['fscRightArrow_outlinedIcon', className && `${className}`].filter(Boolean).join(' ')
  return (
    <svg
      {...props}
...
const RightArrowOutlinedIcon = withLayout(OutlinedIcon)

export { RightArrowOutlinedIcon }
